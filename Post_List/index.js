jQuery.fn.extend({
    ...
    initPostListView: function()
    {
        ...
        const filtersFormSelector = view.data("form");
        const filtersForm = (typeof filtersFormSelector !== "undefined") ? jQuery(filtersFormSelector) : view.find("form");
        const pagedInput = filtersForm.find("input[name='paged']");

        filtersForm.on("wpseedm_submit_ajax_form_before", function(e, data){
        ...
        filtersForm.on("wpseedm_submit_ajax_form_after", function(e, resp, data){
            if(
                resp.status && 
